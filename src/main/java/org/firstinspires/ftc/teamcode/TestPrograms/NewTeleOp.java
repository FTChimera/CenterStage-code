...
@TeleOp(name = "TeleOp", group = "TeleOps")// Name and Group
public class NewTeleOp extends LinearOpMode {// Start of Code
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {// When Program is Running
            ...
            if (gamepad2.dpad_up && leftSlide.getCurrentPosition()<=2000) {// Slides Up
                leftSlide.setPower(1);
                rightSlide.setPower(1);
            } else if (gamepad2.dpad_down && leftSlide.getCurrentPosition()>=0) {// Slides Down
            ...
            if (gamepad1.left_bumper) {// Stop Intaking
                intakeMotor.setPower(0);
                flipDownIntake.setPower(0);
                rollerIntake.setPower(0);
            }

            if (gamepad1.right_trigger > 0.5) {// Outtake
                intakeMotor.setPower(-1);
            }

            if (gamepad1.right_bumper) {// Stop Outtaking
                intakeMotor.setPower(0);
                flipDownIntake.setPower(0);
                rollerIntake.setPower(0);
            }

            if (gamepad2.a){// Box Flips To Intake Position
                outtakeLeft.setPosition(1);
                outtakeRight.setPosition(-1);
            }

            if (gamepad2.b){// Box Flips To Outtake Position
                outtakeLeft.setPosition(-1);
                outtakeRight.setPosition(1);
            }

            if (gamepad2.y){// Flip Down Intake Moves To Non Active Position
                flipDownServo.setPosition(1);
            }
            if (gamepad2.x){// Flip Down Intake Moves To Active Position
                flipDownServo.setPosition(-1);
            }

            if (gamepad1.x){// Close Both Claws
                grip1.setPosition(0.5);
                grip2.setPosition(-0.5);
            }

            if (gamepad1.a){// Open Claw 1
                grip1.setPosition(-0.5);
            }

            if (gamepad1.b){// Open Claw 2
                grip2.setPosition(0.5);
            }
